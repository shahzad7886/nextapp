...
if ($f == 'pages') {
    if ($s == 'create_page') {
        if (!empty($_POST['page_title']) && ($_POST['page_title'] == 'wowonder' || $_POST['page_title'] == 'sunshine' || $_POST['page_title'] == $wo['config']['theme'])) {
            $_POST['page_title'] = "";
        }
        if ( empty($_POST['page_title']) || empty(Wo_Secure($_POST['page_title'])) || Wo_CheckSession($hash_id) === false) {
            $errors[] = $error_icon . $wo['lang']['please_check_details'];
        } else {
            $is_exist = Wo_IsNameExist($_POST['page_title'], 0);
            if (in_array(true, $is_exist)) {
                $errors[] = $error_icon . $wo['lang']['page_name_exists'];
            }
            if (in_array($_POST['page_title'], $wo['site_pages'])) {
                $errors[] = $error_icon . $wo['lang']['page_name_invalid_characters'];
            }
            if (strlen($_POST['page_title']) < 5 OR strlen($_POST['page_title']) > 32) {
                $errors[] = $error_icon . $wo['lang']['page_name_characters_length'];
            }
            if (!preg_match('/^[\w]+$/', $_POST['page_title'])) {
                $errors[] = $error_icon . $wo['lang']['page_name_invalid_characters'];
            }
        ...
        if (empty($errors)) {
            ...
            $re_page_data = array(
                'page_name' => Wo_Secure($_POST['page_title']),
            ...
            if ($register_page) {
                $data = array(
                    'status' => 200,
                    'location' => Wo_SeoLink('index.php?link1=timeline&u=' . Wo_Secure($_POST['page_title']))
                );
            }
        }
        header("Content-type: application/json");
