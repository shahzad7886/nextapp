...
if ($f == 'register') {
    ...
    if (empty($_POST['phone_num']) || empty($_POST['email']) || empty($_POST['username']) || empty($_POST['password']) || empty($_POST['confirm_password']) || empty($_POST['gender'])) {
        $errors = $error_icon . $wo['lang']['please_check_details'];
    } else {
        $is_exist = Wo_IsNameExist($_POST['username'], 0);
        if (empty($_POST['phone_num']) && $wo['config']['sms_or_email'] == 'sms') {
            $errors = $error_icon . $wo['lang']['worng_phone_number'] . " Or Email";
        }
        ...
        if (!empty($_POST['phone_num'])) {
            if (strlen($_POST['phone_num']) < 11) {
                $errors = $error_icon . $wo['lang']['worng_phone_number'] . " Or Email";
            } else if (!preg_match('/^\+?\d+$/', $_POST['phone_num'])) {
                $errors = $error_icon . $wo['lang']['worng_phone_number'] . " Or Email";
            } else {
