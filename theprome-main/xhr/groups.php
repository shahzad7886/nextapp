...
if ($f == 'groups') {
    if ($s == 'create_group' && $wo['config']['can_use_groups']) {
        if (empty($_POST['group_title']) || empty(Wo_Secure($_POST['group_title'])) || Wo_CheckSession($hash_id) === false) {
            $errors[] = $error_icon . $wo['lang']['please_check_details'];
        } else {
            $is_exist = Wo_IsNameExist($_POST['group_title'], 0);
            if (in_array(true, $is_exist)) {
                $errors[] = $error_icon . $wo['lang']['group_name_exists'];
            }
            if (in_array($_POST['group_title'], $wo['site_pages'])) {
                $errors[] = $error_icon . $wo['lang']['group_name_invalid_characters'];
            }
            if (strlen($_POST['group_title']) < 5 OR strlen($_POST['group_title']) > 32) {
                $errors[] = $error_icon . $wo['lang']['group_name_characters_length'];
            }
            if (!preg_match('/^[\w]+$/', $_POST['group_title'])) {
                $errors[] = $error_icon . $wo['lang']['group_name_invalid_characters'];
            }
        ...
        if (empty($errors)) {
            ...
            $re_group_data = array(
                'group_name' => Wo_Secure($_POST['group_title']),
            ...
            if ($register_group) {
                if ($privacy == 2) {
                    $group_id = Wo_GroupIdFromGroupname(Wo_Secure($_POST['group_title']));
                    $user_id  = $wo['user']['id'];
                    $active   = 1;
                ...
                $data = array(
                    'status' => 200,
                    'location' => Wo_SeoLink('index.php?link1=timeline&u=' . Wo_Secure($_POST['group_title']))
                );
            }
        }
        header("Content-type: application/json");
