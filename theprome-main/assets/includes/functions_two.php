...
function Wo_SendSMSMessage($to, $message) {
    ...
    if ($wo["config"]["sms_provider"] == "twilio" && !empty($wo["config"]["sms_twilio_username"]) && !empty($wo["config"]["sms_twilio_password"]) && !empty($wo["config"]["sms_t_phone_number"])) {
        ...
        $result = curl_exec($ch);
        if (curl_errno($ch)) {
            return 'Failed to send SMS: ' . curl_error($ch);
        }
        curl_close($ch);
        ...
        if (!empty($result->Message) && !empty($result->Message->Status)) {
            ...
            return 'Failed to send SMS: ' . $result->Message->Error->Message;
        }
        ...
        if (!empty($result['messages'])) {
            ...
            return 'Failed to send SMS: ' . $result['requestError']['serviceException']['text'];
        }
        ...
        if ($result['http_status'] != 201) {
            return 'Failed to send SMS: ' . $result['http_status'];
        } else {
            return true;
        }
    ...
    return 'SMS provider configuration error';
}
